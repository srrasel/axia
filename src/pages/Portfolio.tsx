...
import { MobileTrades } from '../components/portfolio/MobileTrades'
...
export function PortfolioPage() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col md:flex-row">
        ...
        <div className="flex h-full min-h-0 flex-col md:hidden">
          <div className="flex shrink-0 items-center gap-2 border-b border-border px-3 py-2">
            <button
              ...
              className="rounded-lg px-3 py-2 text-sm font-medium text-text-secondary hover:bg-muted hover:text-text"
            >
              ← Trades
            </button>
            <span className="text-sm font-semibold text-text">
              {section === 'journal' ? 'Trading Journal' : 'Calendar'}
            </span>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto p-4">
            {section === 'journal' ? <TradingJournal /> : <TradingCalendar />}
          </div>
        </div>
      )}

      {/* Desktop sidebar */}
      <div className="panel hidden shrink-0 overflow-x-auto border-b md:flex md:w-52 md:flex-col md:border-b-0 md:border-r">
        {nav.map((item) => (
          <button
            key={item.id}
            type="button"
      ...
      <div className="hidden min-w-0 flex-1 overflow-x-clip overflow-y-auto p-4 sm:p-5 md:block [scrollbar-gutter:stable]">
        {section === 'journal' ? (
          <TradingJournal />
        ) : section === 'calendar' ? (
          <TradingCalendar />
